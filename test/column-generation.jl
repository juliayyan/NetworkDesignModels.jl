...
@testset "Loading Network" begin
    ...
    @test DataFrames.nrow(demand) == 116798
end 
...
demand = DataFrames.by(demand, [:origin, :destination, :hour],
            d -> DataFrames.DataFrame(demand = sum(d[:demand])))
demand = demand[demand[:origin] .!= demand[:destination],:]
@testset "Clustering Stations" begin
    @test sum(demand[:demand]) == 364124
    @test DataFrames.nrow(demand) == 101758
end 
